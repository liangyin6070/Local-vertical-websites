...
public class MsSqlPagingInterceptor extends AbstractPagingInterceptor {
	/**
	 * 改造sql变成查询总数的sql
	 * 
	 * @param targetSql
	...
	protected String getSelectPagingSql(String targetSql, PagingBounds bounds) {
		...
		if (sqlBuilder.indexOf(ORDER_BY) != -1) {
			...
			sqlBuilder.insert(selectPos + SELECT.length(),
					" TOP(" + bounds.getSelectCount() + ")");
		}
		...
		sqlBuilder.insert(0, "WITH query AS (").append(") SELECT ").append("*")
				.append(" FROM query ");
		sqlBuilder.append("WHERE __mybatis_row_nr__ >= "
				+ (bounds.getOffset() + 1) + " AND __mybatis_row_nr__ <= "
				+ bounds.getSelectCount());
